...
describe('hashTable_Tests_', () => {
  ...
  describe('get_Tests_', () => {
    test('get_CatchError_ThrowError', () => {
      // arrange
      hashTable['getBucketLocationForKey'] = jest
        .fn()
    ...
    test('get_KeyFoundNonEmptyHashTable_ReturnEntry', () => {
      ...
      const bucketInsertLocation = hashTable['getBucketLocationForKey'](
        existingValueInHashTable.key
      );
      let bucket = hashTable['hashTable'][bucketInsertLocation];
  ...
  describe('put_Tests_', () => {
    test('put_CatchError_ThrowError', () => {
      ...
      // act / assert
      expect(() => {
        hashTable.put(faker.hacker.noun(), faker.hacker.verb());
      }).toThrowError();
    });
  ...
  describe('remove_Tests_', () => {
    ...
    test('remove_NonEmptyHashTableEntryFound_ReturnRemovedEntry', () => {
      ...
      hashTable.put(existingKey, existingValue);
      const oldSize = hashTable['size'];
  ...
  describe('resize_Tests_', () => {
    test('resize_CatchError_ThrowError', () => {
      ...
      hashTable['logger'].warn = jest.fn().mockImplementationOnce(() => {
        throw new Error();
      });
    ...
    test('resize_LoadFactorThresholdTriggersResize_PerformResize', () => {
      // arrange
      const configuredLoadFactor = 0.7;
      hashTable = new HashTable<string, string>(10, configuredLoadFactor, 2.0);
      ...
      // act
      hashTable.put(
        // indirectly trigger a resize event by adding an entry that will put the table size above it's configured load factor
        faker.hacker.adjective(),
        faker.hacker.phrase()
      );

      // assert
      expect((oldNumberOfEntries + 1) / oldTableSize).toBeGreaterThanOrEqual(
        loadFactor
      );
      expect(hashTable['size']).toEqual(oldNumberOfEntries + 1);
      expect(hashTable['hashTable'].length).toEqual(
        oldTableSize * resizeFactor
      );
  ...
  describe('getBucketLocationForKey_Tests_', () => {
    test('getBucketLocationForKey_ValidInput_ReturnLocation', () => {
      ...
      // act
      const result = hashTable['getBucketLocationForKey'](
        faker.animal.rodent()
      );
